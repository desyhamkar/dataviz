...
``` r
dim(iris) # dimensi object
```

    ## [1] 150   5

``` r
str(iris) # struktur R object
```
...
``` r
head(iris) # 6 baris pertama data iris
```
...
``` r
tail(iris) # 6 baris terakhir dara iris
```
...
``` r
summary(iris) # ringkasan dari data iris untuk masing2 kolom
```
...
``` r
class(iris$Species) # object class
```

    ## [1] "factor"

``` r
length(iris$Petal.Width) # panjang object (jumlahbaris)
```

    ## [1] 150

``` r
levels(iris$Species) # mengetahui jumlah level dalam variable, pada kolom spesies atau nama kategory dari spesies
```

    ## [1] "setosa"     "versicolor" "virginica"

``` r
length(levels(iris$Species)) # panjang object pada kolom iris (jumlah kategorinya 3)
```

    ## [1] 3

``` r
unique(iris$Species) # menampilkan kategori uniq pada kolom species (yaitu : sentosa,versicolor,virginica)
```
...
``` r
length(unique(iris$Species)) # informasi panjang object uniq
```

    ## [1] 3

``` r
mean(iris$Sepal.Length) # rata-rata dari satu kolom ex : kolom sepal.length
```

    ## [1] 5.843333

``` r
sd(iris$Sepal.Length) # standar deviasi 
```

    ## [1] 0.8280661

``` r
median(iris$Sepal.Width) # nilai tengah
```

    ## [1] 3

``` r
sum(iris$Petal.Length) # penjumlahan dari object pada kolom petal.length
```

    ## [1] 563.7

``` r
cor(iris[, -5]) # Correlation, Variance and Covariance (Matrices) antar kolom
```
...
1.  Lakukan select – memilih kolom
2.  memilih baris dengan melakukan filter data ( memfilter kolom tahun)
3.  mutate – membuat kolom /memodifikasi kolom yang sudah ada 4.group by
    – memngelompokkan berdasarkan bulan (pada contoh diatas)
4.  summaries – melakukan perhitungan rata-rata
